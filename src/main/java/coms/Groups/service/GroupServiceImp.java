...
public class GroupServiceImp implements GroupService {
	...
	public List<RegisterUser> getInfo(int groupId) {
		...
		RegisterUser trainer= restTemplate.getForObject("http://localhost:8098/elearning/api/userData/"+gr.getTrainerId(), RegisterUser.class);
		res.add(trainer);
		
ResponseEntity<StudentBean[]> response= restTemplate.getForEntity("http://localhost:9092/elearning/api/student/getParticipants",StudentBean[].class);
			   
		
//		GroupBean[] res = response.getBody();
	List<StudentBean> stud=Arrays.asList(response.getBody());
	for(StudentBean s:stud) {
		RegisterUser st= restTemplate.getForObject("http://localhost:8098/elearning/api/userData/"+s.getUserId(), RegisterUser.class);
		res.add(st);
	}
	return res;
	}
